...
elseif arg[1] == 'handtools' then
    -- run handtool videos
    tool_name = arg[2]
    video_name = arg[3]
    data_dir = '/sequoia/data2/zoli/100_handtool_videos/'
    a = loadAnnotHandtools(video_name)
...
for i = 1,nsamples do
    -- Set up input image
    local im
    if arg[1] == 'handtools' then
        local frame_path = data_dir .. tool_name .. '/frames/' .. video_name .. string.format("/%06d.jpg", a['fids'][idxs[i]])
        im = image.load(frame_path)
    else
...
elseif arg[1] == 'demo' then
    w.window:close()
elseif arg[1] == 'handtools' then
    local save_path = '/sequoia/data3/zoli/contact/baseline/3d_pose_baseline/' .. video_name .. '/pose_2d.h5'
    local predFile = hdf5.open(save_path, 'w')
    predFile:write('preds', preds)
    predFile:close()
end
